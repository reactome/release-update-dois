...
public class TestUpdateDOIs {
	...
	public void testfindNewDOIsAndUpdate() throws Exception {
		...
		Mockito.when(instance.getAttributeValue("name"))
			.thenReturn("Name Value");
		Mockito.when(instance.getAttributeValue("DB_ID"))
			.thenReturn("67890");

		Mockito.when(mockAdaptor.fetchInstanceByAttribute("Pathway", "doi", "NOT REGEXP", "^10.3180"))
			.thenReturn(testResults);
		Mockito.when(mockAdaptor.fetchInstanceByAttribute("Pathway", "DB_ID", "=", "67890"))
			.thenReturn(testResults);
